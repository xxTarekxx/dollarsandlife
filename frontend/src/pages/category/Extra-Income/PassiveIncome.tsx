...
const PassiveIncome: React.FC = () => {
	const breadcrumbPaths = [
		...
		{ title: "Passive Income", url: "/jobs/Passive-Income" },
	];

	const passiveincomejob = [
		{
			id: 1,
	...
	const blogPostRows = [];
	for (let i = 0; i < passiveincomejob.length; i += 3) {
		blogPostRows.push(passiveincomejob.slice(i, i + 3));
	}
		...
		<PageContainer>
			...
			{blogPostRows.map((row, rowIndex) => (
				<RowContainer key={rowIndex}>
					...
					<BlogPostWrapper>
						{row.map((passiveincomejob, index) => (
							<React.Fragment key={passiveincomejob.id}>
								<BlogPost
									id={passiveincomejob.id}
									title={passiveincomejob.title}
									imageUrl={passiveincomejob.imageUrl}
									content={passiveincomejob.content}
									author={passiveincomejob.author}
									datePosted={passiveincomejob.datePosted}
								/>
								{(index + 1) % 3 === 0 && (
