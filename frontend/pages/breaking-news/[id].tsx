...
export const getServerSideProps: GetServerSideProps = async (context) => {
	...
	try {
		const response = await fetch(
			...
			}/breaking-news/${encodeURIComponent(id)}?lang=${locale}`,
		);
		if (!response.ok) {
...
const BreakingNewsDetail: React.FC<BreakingNewsDetailProps> = ({
	...
	const canonicalUrl = `https://www.dollarsandlife.com${langPrefix}/breaking-news/${post.id}`;
			...
			<Head>
				...
				{(post.availableLangs ?? ['en']).map((lang) => (
					<link key={lang} rel='alternate' hrefLang={lang}
						href={lang === 'en'
							? `https://www.dollarsandlife.com/breaking-news/${post.id}`
							: `https://www.dollarsandlife.com/${lang}/breaking-news/${post.id}`
						} />
				))}
				<link rel='alternate' hrefLang='x-default' href={`https://www.dollarsandlife.com/breaking-news/${post.id}`} />
				{/* Open Graph */}
