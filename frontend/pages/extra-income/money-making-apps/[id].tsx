...
export const getServerSideProps: GetServerSideProps = async (context) => {
	...
	try {
		const response = await fetch(
			...
			}/money-making-apps/${encodeURIComponent(id)}?lang=${locale}`,
		);
		if (!response.ok) {
...
const MoneyMakingAppDetail: React.FC<MoneyMakingAppDetailProps> = ({
	...
	const canonicalUrl = `https://www.dollarsandlife.com${langPrefix}/extra-income/money-making-apps/${post.id}`;
		...
		<div className='page-container'>
			<Head>
				...
				{(post.availableLangs ?? ['en']).map((lang) => (
					<link key={lang} rel='alternate' hrefLang={lang}
						href={lang === 'en'
							? `https://www.dollarsandlife.com/extra-income/money-making-apps/${post.id}`
							: `https://www.dollarsandlife.com/${lang}/extra-income/money-making-apps/${post.id}`
						} />
				))}
				<link rel='alternate' hrefLang='x-default' href={`https://www.dollarsandlife.com/extra-income/money-making-apps/${post.id}`} />
				{/* Open Graph */}
			...
			<nav className='related-reading' aria-label='Related reading'>
				...
				<ul className='related-reading-list'>
					...
					<li><Link href='/shopping-deals' className='related-reading-link'>Shopping Deals & Savings</Link></li>
