...
export const getServerSideProps: GetServerSideProps = async (context) => {
	...
	try {
		const response = await fetch(
			...
			}/budget-data/${encodeURIComponent(id)}?lang=${locale}`,
		);
		if (!response.ok) {
		...
		if (firstTextSection && typeof firstTextSection.text === "string") {
			...
			post.metaDescription = "Detailed budget post.";
		}
...
const BudgetDetail: React.FC<BudgetDetailProps> = ({
	...
	const canonicalUrl = `https://www.dollarsandlife.com${langPrefix}/extra-income/budget/${post.id}`;
		...
		<div className='page-container'>
			<Head>
				...
				{(post.availableLangs ?? ['en']).map((lang) => (
					<link key={lang} rel='alternate' hrefLang={lang}
						href={lang === 'en'
							? `https://www.dollarsandlife.com/extra-income/budget/${post.id}`
							: `https://www.dollarsandlife.com/${lang}/extra-income/budget/${post.id}`
						} />
				))}
				<link rel='alternate' hrefLang='x-default' href={`https://www.dollarsandlife.com/extra-income/budget/${post.id}`} />
				{/* Open Graph */}
			...
			<nav className='related-reading' aria-label='Related reading'>
				...
				<ul className='related-reading-list'>
					...
					<li><Link href='/extra-income' className='related-reading-link'>More Extra Income Guides</Link></li>
				</ul>
			</nav>
		</div>
	);
