...
const getPost = cache(async (lang: string, id: string) => {
	const res = await fetch(
		`${INTERNAL_API}/remote-jobs/${encodeURIComponent(id)}?lang=${lang}`,
	...
	try {
		...
		const description =
			first && typeof first.text === "string"
				...
				: "Detailed remote job post.";
		return {
			title: title ? `${title} | Remote Jobs` : "Remote Jobs",
			description,
		};
	} catch {
		return {};
	...
	try {
		...
		post.metaDescription =
			first && typeof first.text === "string"
				? sanitizeAndTruncateHTML(first.text, 160)
				: "Detailed remote job post.";
		const Component = (await import("@pages/extra-income/remote-online-jobs/[id]")).default;
